...
import de.tobi.bank.util.action.AccountAction;
...
public class YourAccountsAction implements InventoryAction, BedrockUIAction<SimpleForm> {

    private final CompletableFuture<List<Account>> accounts;

    public YourAccountsAction(@NotNull UUID uuid) {
        this.accounts = BankSystem.getInstance().getAccounts(uuid);
    }
    ...
    public @Nullable Action execute(@NotNull Player player, @NotNull SimpleForm form, @NotNull FormResponse formResponse) {
        ...
        Account account = this.accounts.join().get(simpleFormResponse.clickedButtonId() - 1);
        if (account == null) {
            return null;
        }
    ...
    public @Nullable FormBuilder<?, SimpleForm, ?> buildForm(@NotNull Player player) {
        SimpleForm.Builder builder = SimpleForm
                ...
                .button(LanguageApi.getMessage(player.getName(), "grieferprime.banksystem.youraccounts.create"));
    ...
    public @NotNull String getDefaultTitle(@NotNull Player player) {
        return LanguageApi.getMessage(player.getName(), "grieferprime.banksystem.youraccounts.title");
    }
    ...
    public void fillInventory(@NotNull Player player, @NotNull Inventory inventory) {
        ...
        inventory.setItem(49, new ItemBuilder(Material.EMERALD).setName(LanguageApi.getMessage(player.getName(), "grieferprime.banksystem.youraccounts.create")).build());
    }
}
